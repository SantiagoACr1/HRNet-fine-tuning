...
parser.add_argument('--ckpt_path',  type=str,
                    help='ckpt path', default='./log/normal_baseline/hrnetw32/model-5000.pth')
...
def predict_test(ckpt_path, config_path='baseline.hrnetw32', save_dir=''):
    ...
    if 'model' in statedict:
        model_state_dict = statedict['model']
    else:
    ...
    with torch.no_grad():
        for img, gt in tqdm(test_dataloader):
            ...
            for clsmap, imname in zip(pred, gt['fname']):
                imsave(os.path.join(save_dir, imname), clsmap.numpy().astype(np.uint8))
